...
pub struct BlueprintEditorPanel {
    ...
    pub library_manager: ui::graph::LibraryManager,
    // Local macros defined within this blueprint class
    pub local_macros: Vec<ui::graph::SubGraphDefinition>,
...
impl BlueprintEditorPanel {
    ...
    pub fn compile_to_rust(&self) -> Result<String, String> {
        ...
        ui::compiler::compile_graph(&graph_description)
    }
    ...
    pub fn compile_to_class_directory(&self) -> Result<(), String> {
        ...
        let data_resolver = ui::compiler::data_resolver::DataResolver::build_with_variables(
            &graph_description,
            &metadata,
            variables.clone(),
        )?;
        let exec_routing = ui::compiler::execution_routing::ExecutionRouting::build_from_graph(
            &graph_description,
        );
        ...
        for event_node in &event_nodes {
            ...
            let mut generator = ui::compiler::code_generator::CodeGenerator::new(
                &metadata,
                &data_resolver,
                &exec_routing,
                &graph_description,
    ...
    fn convert_to_graph_description(&self) -> Result<ui::graph::GraphDescription, String> {
        self.convert_graph_to_description(&self.graph)
    }
    
    /// Convert a specific blueprint graph to graph description format
    fn convert_graph_to_description(&self, graph: &BlueprintGraph) -> Result<ui::graph::GraphDescription, String> {
        use ui::graph::*;
    ...
    pub fn save_blueprint(&mut self, file_path: &str) -> Result<(), String> {
        ...
        let variables: Vec<ui::graph::ClassVariable> = self.class_variables.iter().map(|v| {
            ui::graph::ClassVariable {
                id: uuid::Uuid::new_v4().to_string(), // Generate ID for compatibility
                name: v.name.clone(),
                data_type: ui::graph::DataType::String, // TODO: parse var_type properly
        ...
        let blueprint_asset = ui::graph::BlueprintAsset {
            format_version: 1,
    ...
    fn sync_all_tabs_to_storage(&mut self) {
        ...
        let mut converted_graphs: Vec<(String, ui::graph::GraphDescription)> = Vec::new();
    ...
    pub fn load_blueprint(
        ...
        if let Ok(blueprint_asset) = serde_json::from_str::<ui::graph::BlueprintAsset>(&json) {
            // New unified format
    ...
    fn convert_from_graph_description(
        &mut self,
        graph_desc: &ui::graph::GraphDescription,
        window: &mut gpui::Window,
                    ...
                    .map(|pin_inst| {
                        ...
                        Pin {
                            ...
                            pin_type: match pin.pin_type {
                                ui::graph::PinType::Input => PinType::Input,
                                ui::graph::PinType::Output => PinType::Output,
                            },
                ...
                properties: node_instance
                    ...
                    .map(|(k, v)| {
                        let value_str = match v {
                            ui::graph::PropertyValue::String(s) => s.clone(),
                            ui::graph::PropertyValue::Number(n) => n.to_string(),
                            ui::graph::PropertyValue::Boolean(b) => b.to_string(),
                            _ => "".to_string(),
                        };
                        (k.clone(), value_str)
                    })
    ...
    fn propagate_reroute_types(
        ...
        if data_type == ui::graph::DataType::Any {
            return;
        }
            ...
            let properties: std::collections::HashMap<String, String> = node_instance.properties.iter()
                .map(|(k, v)| {
                    let value_str = match v {
                        ...
                        ui::graph::PropertyValue::Number(n) => n.to_string(),
                        ui::graph::PropertyValue::Boolean(b) => b.to_string(),
                        ui::graph::PropertyValue::Vector2(x, y) => format!("({}, {})", x, y),
                        ui::graph::PropertyValue::Vector3(x, y, z) => format!("({}, {}, {})", x, y, z),
                        ui::graph::PropertyValue::Color(r, g, b, a) => format!("({}, {}, {}, {})", r, g, b, a),
                    };
                    (k.clone(), value_str)
                })
                .collect();
    ...
    pub fn create_new_local_macro(&mut self, cx: &mut Context<Self>) {
        ...
        let graph_desc = ui::graph::GraphDescription::new(&macro_name);
        ...
        let macro_def = ui::graph::SubGraphDefinition {
            ...
            interface: ui::graph::SubGraphInterface {
                inputs: vec![ui::graph::SubGraphPin {
                    id: "exec_in".to_string(),
                    name: "Body".to_string(),
                    data_type: ui::graph::DataType::Execution,
                    description: Some("Execution input".to_string()),
                    default_value: None,
                    is_instance_editable: false,
                    category: None,
                }],
                outputs: vec![ui::graph::SubGraphPin {
                    id: "exec_out".to_string(),
                    name: "Then".to_string(),
                    data_type: ui::graph::DataType::Execution,
            ...
            macro_config: ui::graph::MacroConfiguration {
                is_pure: false,
