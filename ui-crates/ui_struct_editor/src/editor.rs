...
use ui_types_common::StructAsset;
use std::path::PathBuf;

pub struct StructEditor {
    file_path: Option<PathBuf>,
    asset: Option<StructAsset>,
        ...
        let (asset, error_message) = match std::fs::read_to_string(&file_path) {
            Ok(json_content) => {
                match serde_json::from_str::<StructAsset>(&json_content) {
                    ...
                    Err(e) => (None, Some(format!("Failed to parse struct: {}", e))),
                }
            }
                                ...
                                div()
                                    ...
                                    .child(
                                        self.asset.as_ref()
                                            ...
                                            .unwrap_or_else(|| "Struct Editor".to_string())
                                    )
                            )
                    )
