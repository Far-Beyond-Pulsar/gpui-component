...
impl TraitEditor {
    pub fn new(project_root: PathBuf, _window: &mut Window, cx: &mut Context<Self>) -> Self {
        ...
        let asset = TraitAsset {
            ...
            display_name: "NewTrait".to_string(),
            description: None,
            methods: vec![],
    ...
    pub fn open(file_path: PathBuf, project_root: PathBuf, _window: &mut Window, cx: &mut Context<Self>) -> anyhow::Result<Self> {
        ...
        let asset: TraitAsset = serde_json::from_str(&json_content)?;
    ...
    pub fn save(&mut self, cx: &mut Context<Self>) -> anyhow::Result<()> {
        validate_trait(&self.asset, &self.type_index)?;

        let file_path = if let Some(path) = &self.file_path {
            path.clone()
        } else {
            self.index_manager.ensure_type_dir(TypeKind::Trait, &self.asset.name)?;
            self.index_manager.get_json_path(TypeKind::Trait, &self.asset.name)
        };

        let json = serde_json::to_string_pretty(&self.asset)?;
        std::fs::write(&file_path, json)?;

        let rust_code = generate_trait(&self.asset)?;
        let rs_path = self.index_manager.get_rs_path(TypeKind::Trait, &self.asset.name);
        std::fs::write(&rs_path, rust_code)?;

        let mut index = self.index_manager.load_index().unwrap_or_default();
        let entry = TypeIndexEntry::new(TypeKind::Trait, self.asset.name.clone(), self.asset.display_name.clone());
        index.upsert(TypeKind::Trait, entry)?;
        ...
        cx.emit(TraitEditorEvent::TraitSaved(self.asset.name.clone()));
        cx.notify();
                        ...
                        Button::new("save")
                            ...
                            .on_click(cx.listener(|editor, _, _, cx| {
                                if let Err(e) = editor.save(cx) {
                                    eprintln!("Failed to save trait: {}", e);
                                }
                            }))
                    )
            )
                ...
                v_flex()
                    ...
                    .child(
                        div()
                            .text_sm()
                            .text_color(cx.theme().foreground)
                            .child(format!("Trait Editor: {}", self.asset.name))
                    )
            )
    }
}
