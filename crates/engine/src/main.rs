#[cfg(target_os = "linux")]
use crate::settings::engine_settings;
...
use ui::loading_window::{LoadingComplete, LoadingWindow};
...
struct WinitGpuiApp {
    ...
    #[cfg(target_os = "linux")]
    swap_chain: Option<std::sync::Arc<wgpu::Surface<'static>>>,
    #[cfg(target_os = "windows")]
...
impl ApplicationHandler for WinitGpuiApp {
            ...
            match event {
                ...
                WindowEvent::RedrawRequested => {
                    ...
                    unsafe {
                        ...
                        if !self.shared_texture_initialized
                            && self.gpui_window.is_some()
                            && self.gpui_app.is_some()
                            && self.d3d_device.is_some()
                        {
                            ...
                            if let Ok(handle_ptr) = handle_result {
                                if let Some(handle_ptr) = handle_ptr {
                                    println!(
                                        "✅ Got shared texture handle from GPUI: {:p}",
                                        handle_ptr
                                    );
                                    ...
                                    let result = device
                                        .OpenSharedResource(HANDLE(handle_ptr as _), &mut texture);
                                    ...
                                    match result {
                                        Ok(_) => {
                                            if let Some(shared_texture) = texture {
                                                ...
                                                desc.MiscFlags =
                                                    D3D11_RESOURCE_MISC_FLAG(0).0 as u32; // Remove shared flag
                                                desc.Usage = D3D11_USAGE_DEFAULT;
                                                desc.BindFlags =
                                                    D3D11_BIND_SHADER_RESOURCE.0 as u32;

                                                let mut persistent_texture: Option<
                                                    ID3D11Texture2D,
                                                > = None;
                                                let create_result = device.CreateTexture2D(
                                                    &desc,
                                                    None,
                                                    Some(&mut persistent_texture),
                                                );

                                                if create_result.is_ok()
                                                    && persistent_texture.is_some()
                                                {
                                                    ...
                                                    let mut srv: Option<ID3D11ShaderResourceView> =
                                                        None;
                                                    let srv_result = device
                        ...
                        if let (
                            ...
                            // Use cached SRV (no per-frame allocation!)
                            {
                                ...
                                if FRAME_COUNT % 60 == 1 {
                                    eprintln!(
                                        "🎨 Compositing GPUI texture (frame {})",
                                        FRAME_COUNT
                                    );
                                }
                            ...
                            if SKIP_COUNT <= 3 || SKIP_COUNT % 60 == 0 {
                                eprintln!(
                                    "⏭️  Skipping frame {} - waiting for GPUI texture to be ready",
                                    SKIP_COUNT
                                );
                            }
                        }
                ...
                WindowEvent::KeyboardInput { event, .. } => {
                    ...
                    if let (Some(gpui_window), Some(gpui_app)) =
                        (&self.gpui_window, &mut self.gpui_app)
                    {
                        ...
                        if let Some(keystroke) = keystroke_opt {
                            ...
                            let _ = gpui_app
                                .update(|cx| gpui_window.inject_input_event(cx, gpui_event));
                        }
                    }
                ...
                WindowEvent::ModifiersChanged(new_modifiers) => {
                        ...
                        (&self.gpui_window, &mut self.gpui_app)
                    {
                        ...
                        let _ =
                            gpui_app.update(|cx| gpui_window.inject_input_event(cx, gpui_event));
                    }
                ...
                WindowEvent::Resized(new_size) => {
                    ...
                    if let Some(swap_chain) = &self.swap_chain {
                        unsafe {
                            ...
                            #[cfg(target_os = "linux")]
                            let resize_result = swap_chain.configure(
                            ...
                            let mut _handle_success =
                                || {
                                    println!(
                                        "✅ Resized swap chain to {}x{}",
                                    ...
                                    if let Some(device) = &self.d3d_device {
                                        #[cfg(target_os = "windows")]
                                    let back_buffer: Option<
                                        ID3D11Texture2D,
                                    > = swap_chain.GetBuffer(0).ok();
                                        #[cfg(target_os = "linux")]
                                    let back_buffer: Option<
                                        wgpu::SurfaceTexture,
                                    > = swap_chain.get_current_texture().ok();
                                        if let Some(back_buffer) = back_buffer {
                                            #[cfg(target_os = "windows")]
                                        let mut rtv: Option<ID3D11RenderTargetView> = None;
                                            #[cfg(target_os = "linux")]
                                        let rtv: Option<wgpu::TextureView> = None;
                                            ...
                                            #[cfg(target_os = "linux")]
                                            let _ = back_buffer.texture.create_view(
                                                &wgpu::TextureViewDescriptor {
                                                    label: Some("Big titty bitches"),
                ...
                WindowEvent::CursorMoved { position, .. } => {
                    ...
                    if let Some(winit_window) = &self.winit_window {
                        ...
                        // Debug output
                        eprintln!(
                            "🖱️ CursorMoved: physical ({}, {}), logical ({}, {})",
    ...
    fn about_to_wait(&mut self, _event_loop: &ActiveEventLoop) {
        ...
        if !self.gpui_window_initialized && self.winit_window.is_some() && self.gpui_app.is_some() {
            ...
            println!(
                "🎯 Creating GPUI window: physical {}x{}, scale {}, logical {}x{}",
            ...
            {
                let gpui_window = app
                    .open_window_external(external_handle.clone(), |window, cx| {
                ...
                unsafe {
                    ...
                    if result.is_ok() && device.is_some() {
                        ...
                        println!(
                            "🎯 Creating swap chain: physical {}x{}, scale {}",
                            physical_width,
                            physical_height,
                        ...
                        if let Ok(swap_chain) = swap_chain {
                            ...
                            if device
                                .as_ref()
                                .unwrap()
                                .CreateBlendState(&blend_desc, Some(&mut blend_state as *mut _))
                                .is_ok()
                            {
...
"#;
                            ...
                            if vs_result.is_ok() && ps_result.is_ok() {
                                ...
                                println!(
                                    "❌ Failed to create shaders - VS: {:?}, PS: {:?}",
                                    vs_result, ps_result
                                );
                            }
                            ...
                            if device
                                ...
                                .CreateSamplerState(
                                    &sampler_desc,
                                    Some(&mut sampler_state as *mut _),
                                )
                                .is_ok()
                            {
            ...
            #[cfg(target_os = "linux")]
            {
                // This function MUST use the standard 'open_window' method.

                // Calculate logical bounds etc (the same logic you had before)
                let scale_factor = winit_window.scale_factor() as f32;
                let size = winit_window.inner_size();
                let logical_width = size.width as f32 / scale_factor;
                let logical_height = size.height as f32 / scale_factor;

                // Use the *standard* open_window method, which works internally on Linux
                app.update(|app_context| {
                    let _ = app_context.open_window(
                        gpui::WindowOptions {
                            display_id: None,
                            ...
                            window_bounds: Some(gpui::WindowBounds::Windowed(gpui::Bounds {
                                ...
                                size: Size {
                                    width: Pixels::from(logical_width),
                                    height: Pixels::from(logical_height),
                                },
                            })),
                            titlebar: Some(TitlebarOptions {
                                title: Some(SharedString::new("Test App")),
                            ...
                            app_id: Some(String::from("Test")),
                            window_min_size: Some(Size {
                                width: Pixels::from(100 as f32),
                                height: Pixels::from(100 as f32),
                            }),
                ...
                println!("✅ GPUI window initialized using standard flow for Linux.");
            }
